...
var (
	kafkaBrokers = "10.210.27.95:9092,10.210.27.99:9092,10.210.27.98:9092"
	kafkaTopic   = "logCollecter"
...
func (w *fileLogWriter) write(level Level, s string) {
	...
	/*	if level != LevelAction {
		inputKafka(buf.Bytes(), string(level))
	}*/
...
func kafkaInit() {
	var err error
	producer, err = kafka.NewAsyncProducer(kafkaBrokers)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func inputKafka(buf []byte, key string) {
	err := producer.Send(kafkaTopic, key, buf)
	if err != nil {
		fmt.Println("kafkaInput err:", err)
	}
}
*/
